            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0">Sub Footer Category
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            @if(isset($item))
                            <form class="needs-validation" action="{{route('subFooter.update',$item->id)}}" method="POST" enctype="multipart/form-data">
                                @method('PATCH')
                                @else
                                <form class="needs-validation" action="{{route('subFooter.store')}}" method="POST" enctype="multipart/form-data">
                                    @endif
                                    @csrf
                                    <div class="row g-2">
                                        <div class="col-md-3">
                                            <label for="invoicenoInput">Category Name*</label>
                                            <select class="form-select " name="parent_id" aria-label="Default select example">
                                                <option value="0">select Category</option>

                                                @foreach($cat as $row)
                                                <option @if (isset($item)) {{ $item->parent_id === $row->id ? 'selected' : '' }} @else {{ old('parent_id') === $row->id ? 'selected' : '' }} @endif value="{{$row->id}}">{{$row->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-3">
                                            <label for="invoicenoInput">Sub Category Name*</label>
                                            <input type="text" name="name" class="form-control bg-light " id="invoicenoInput" value="{{isset($item)?$item->name:''}}">

                                        </div>
                                        <div class="col-md-3">
                                            <label for="invoicenoInput">Sub Category Link*</label>
                                            <input type="text" name="link" class="form-control bg-light " id="invoicenoInput" value="{{isset($item)?$item->link:''}}">
                                    ...
                                    <table class="table table-bordered align-middle table-nowrap mb-0">
                                        <thead class="table-light">
                                            <tr>
                                                ...
                                                <th scope="col">Sub Category Link</th>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php
                                                    $par = Cpr_footer_category::find($data->parent_id);
                                                    ?>
                                                    {{$par->name}}
                                                </td>
                                                <td>{{$data->name}}</td>
                                                <td><a href="{{$data->link}}" target="_blank" rel="noopener noreferrer">{{$data->link}}</a></td>
                                                <td><input type="number" min="1" onkeypress="if(this.value.length===8) return false;" name="sortid[]" value="{{$data->sort}}" class="form-control"></td>
                                                ...
                                                <td align="right">
                                                    <ul class="list-inline hstack gap-2 mb-0">
                                                        <li class="list-inline-item edit" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="{{$data->status==1?'Desable':'Enable'}}">
                                                            @if($data->status==1)
                                                            <a href="{{url('subFooter_disable/'.$data->id)}}" class="text-muted d-inline-block">
                                                                <i class="ri-check-fill fs-16" style="color:red;font-weight: bold;"></i>
                                                            </a>
                                                            @endif
                                                            @if($data->status==0)
                                                            <a href="{{url('subFooter_enable/'.$data->id)}}" class="text-muted d-inline-block">
                                                        ...
                                                        <li class="list-inline-item">
                                                            <a class="edit-item-btn" href="{{route('subFooter.edit',$data->id)}}" href="javascript:void(0)"><i class="ri-pencil-fill align-bottom text-muted"></i></a>
                                                        </li>
                                                        @if(auth()->user()->delete_access == 1)
                                                        <li class="list-inline-item" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Delete">
                                                            <a class="remove-item-btn" href="{{url('subFooter_delete/'.$data->id)}}" onclick="return confirm('Are You Sure')">
