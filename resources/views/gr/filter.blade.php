            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0">Add Filter </h4>
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            @if(isset($item))
                            <form class="needs-validation" action="{{route('Filter.update',$item->id)}}" method="POST" enctype="multipart/form-data">
                                @method('PATCH')
                                @else
                                <form class="needs-validation" action="{{route('Filter.store')}}" method="POST" enctype="multipart/form-data">
                                    @endif
                                    @csrf
                                    <div class="row g-2">
                                        <div class="col-md-2">
                                            <label for="invoicenoInput">Ad Filter Name*</label>
                                            <input type="text" name="filter_name" class="form-control bg-light " id="invoicenoInput" value="{{isset($item)?$item->filter_name:''}}">

                                        </div>
                                        <div class="col-md-2">
                                            <label for="invoicenoInput">Ad Filter Alias*</label>
                                            <input type="text" name="alias" class="form-control bg-light " id="invoicenoInput" value="{{isset($item)?$item->alias:''}}">

                                        </div>
                                        <div class="col-md-2">
                                            <label for="invoicenoInput">Dropdown Type*</label>
                                            <select class="form-select bg-light " name="type" aria-label="Default select example">
                                                <option value="0">select</option>
                                                <option @if (isset($item)) {{ $item->type == 'single' ? 'selected' : '' }} @else {{ old('type') === 'single' ? 'selected' : '' }} @endif value="single">Single</option>
                                                <option @if (isset($item)) {{ $item->type == 'multiple' ? 'selected' : '' }} @else {{ old('type') === 'multiple' ? 'selected' : '' }} @endif value="multiple">Multiple</option>

                                            </select>
                                        </div>
                                        <div class="col-md-2">
                                            <label for="invoicenoInput">Description*</label>
                                            <input type="text" name="description" class="form-control bg-light " id="invoicenoInput" value="{{isset($item)?$item->description:''}}">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered align-middle table-nowrap mb-0">
                                        <thead class="table-light">
                                            <tr>
                                                <th scope="col">#</th>
                                                <th scope="col">Ad Filter Name</th>
                                                <th scope="col">Alias</th>
                                                <th scope="col">Type</th>
                                                <th scope="col">Description</th>
                                                <!--<th scope="col">Ad Filter Values</th>-->
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <!--<td></td>-->
                                                <td>{{$data->status}}</td>
                                                <td><a href="javascript:;" onclick='setTableData(this)' title='View filters' data-bs-toggle="modal" data-bs-target="#table-modal" data-type='filter' data-table-data=''><i class="mdi-account-eye"></i></a></td>
                                                <td align="right">
                                                    <ul class="list-inline hstack gap-2 mb-0">
                                                        <li class="list-inline-item edit" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="{{$data->status==1?'Desable':'Enable'}}">
                                                            @if($data->status==1)
                                                            <a href="{{url('Filter_desable/'.$data->id)}}" class="text-muted d-inline-block">
                                                                <i class="ri-check-fill fs-16" style="color:red;font-weight: bold;"></i>
                                                            </a>
                                                            @endif
                                                            @if($data->status==0)
                                                            <a href="{{url('Filter_enable/'.$data->id)}}" class="text-muted d-inline-block">
                                                        ...
                                                        <li class="list-inline-item">
                                                            <a class="edit-item-btn" href="{{route('Filter.edit',$data->id)}}" href="javascript:void(0)"><i class="ri-pencil-fill align-bottom text-muted"></i></a>
                                                        </li>
                                                        @if(auth()->user()->delete_access == 1)
                                                        <li class="list-inline-item" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Delete">
                                                            <a class="remove-item-btn" href="{{url('Filter_delete/'.$data->id)}}" onclick="return confirm('Are You Sure')">
