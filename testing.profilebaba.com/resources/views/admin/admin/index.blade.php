			...
			<tbody>
				...
				<tr>
					...
					<td style="text-align: center">
						@if($data->profile_pic != '')
							<img src="{{ url('/uploads/admin/photo/' . $data->profile_pic) }}" style="max-width:24px; margin: auto; display: inline-block;"/>
						@else
							N/A
						@endif
					</td>
					...
					<td>{{ $data->name }}</td>
					<td>{{ $data->email }}</td>
					<td style="text-transform: capitalize;">{{ $data->role->name }}</td>
					<td>
						{{ date('d-m-Y', strtotime($data->created_at)) }}
					</td>
					<td class="text-center">
						<a class="btn btn-primary btn-xs" href="{{ route('admin_log',$data->id) }}">
							Logs
						</a>
